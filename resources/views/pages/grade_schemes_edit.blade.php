...
@section('title', 'Edit Grade Scheme')
...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('grade-schemes.update', $gradeScheme) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="name">Scheme Name</label>
                <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $gradeScheme->name) }}" required>
            </div>

            <div class="form-group">
                <label for="description">Description</label>
                <textarea name="description" id="description" class="form-control" rows="3">{{ old('description', $gradeScheme->description) }}</textarea>
            </div>

            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $gradeScheme->is_active) ? 'checked' : '' }}>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm" id="itemsTable">
                    ...
                    <tbody>
                        @php($rows = old('items', $gradeScheme->items->map(fn($i) => ['letter_grade'=>$i->letter_grade,'gpa'=>$i->gpa,'min_mark'=>$i->min_mark,'max_mark'=>$i->max_mark,'sort_order'=>$i->sort_order])->toArray()))
